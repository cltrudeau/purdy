...
class TestStyle(TestCase):
    def test_style_init(self):
        # The rest of the tests hit most of the Style class, this just tickles
        # the bits that don't get run
        theme = Theme({})
        style = Style(Code.text("", "plain"), theme)
        self.assertEqual(theme, style.theme)


    def test_folding(self):
        # Sample text numbers 1-10 separated by newlines
        text = "\n".join([str(x) for x in range(0, 10)]) + "\n"
        style = Style(Code.text(text, "plain"))

        # Fold lines 2 through 9
        style.fold(1, 8)
        output = to_plain(style)
        expected = "0\n⠇\n9\n"
    ...
    def test_partial_highlight_chopping(self):
        ...
        result = Style._chop_partial_highlight(line, [(4, 5)])
        ...
        result = Style._chop_partial_highlight(line, [(3, 2)])
        ...
        result = Style._chop_partial_highlight(line, [(7, 6)])
        self.assertEqual(9, len(result.parts))
        ...
        result = Style._chop_partial_highlight(line, [(3, 8)])
        self.assertEqual(8, len(result.parts))
        self.assertEqual(HighlightOn, result.parts[2].token)
        ...
        result = Style._chop_partial_highlight(line, [(3, 2), (7, 2)])
    ...
    def test_decorate(self):
        ...
        # applies a lot of style to make sure everything gets tickled
        path = (Path(__file__).parent / Path("data/simple.py")).resolve()
        style = Style(Code(path))
        style.wrap = 35
        style.line_numbers_enabled = True
