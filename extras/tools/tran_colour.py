...
# Modified version of Micah Elliott's excellent code; this version allows for
# python 3 and also prints out urwid's weird length-3 hex shortcuts
...
def convert_to_urwid(index, rgb):
    if index <= 8 or index >= 232:
        return ''

    u = '-%s--%s--%s-' % (rgb[0:2], rgb[2:4], rgb[4:6])
    u = u.replace('-00-', '0')
    u = u.replace('-5f-', '6')
    u = u.replace('-87-', '8')
    u = u.replace('-af-', 'a')
...
def print_all():
    ...
    for short, rgb in CLUT:
        ...
        sys.stdout.write('\033[38;5;%sm%s:%s %s' % (short, short, rgb,
            urwid_rgb))
        sys.stdout.write("\033[0m\n")
