...
from purdy.parser import (CodeLine, CodePart, HighlightOn, HighlightOff, Fold,
    LineNumber)
...
class Motif:
    ### Provides styling to Code, avoiding the word "style" as that means
    # other things in the TUI and that just gets confusing

    def __init__(self, code, theme=None):
        """Class represents styling applied to a :class:`Code` object.

        :param code: `Code` object containing lines of code to apply motif to
        :param theme: :class:`Theme` object that specifies colour map
        """
        self.wrap = None
        ...
        self.code = code
        if theme is None:
            self.theme = THEME_MAP["default"][code.parser.spec.category]
        else:
            self.theme = theme

    def reset_metadata(self):
        """Empties out all metadata associated with this motif. Mostly used
        for testing.
        """
        self.meta = defaultdict(_CodeLineMetadata)
    ...
    def decorate(self):
        ...
        producing a new `Code` object.

        :returns: `Code` object with decoration tokens
        """
